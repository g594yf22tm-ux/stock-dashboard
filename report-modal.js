...
function openReportInline(filename, stockCode, stockName) {
  ...
  Promise.all([
    ...
    _renderReportContent(panel, filename, stockCode, stockName, results[0], results[1], results[2]);
  }).catch(function(err) {
...
function _getStockSnapshot(code) {
  ...
  if (typeof _extQuotes !== 'undefined' && _extQuotes && _extQuotes[code]) {
    ...
    return Promise.resolve({ ticker: code, name: '', price: q.p, changePercent: q.c, volume: q.v, amount: q.a });
  }
...
function markdownToHtml(md) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (!trimmed && !inTable) { out.push('<br>'); continue; }
    if (!trimmed && inTable) { out.push('</tbody></table>'); inTable = false; continue; }
    ...
    if (/^\|.+\|$/.test(trimmed)) {
      ...
      out.push('<tr>' + cells.map(function(c) { return '<' + tag + '>' + _mdInline(c.trim()) + '</' + tag + '>'; }).join('') + '</tr>');
      continue;
    }
...
function _renderReportContent(panel, filename, code, name, reportMd, stockInfo, newsData) {
  ...
  if (newsData && newsData.length) {
    newsHtml = newsData.map(function(n) {
      return '<div class="report-news-item"><a href="' + (n.link||n.url||'#') + '" target="_blank" rel="noopener">📰 ' + (n.title||'') + '</a><div class="rn-date">' + (n.date||n.publishedAt||'') + (n.source ? ' | ' + n.source : '') + '</div></div>';
    }).join('');
  } else {
    newsHtml = '<div style="text-align:center;padding:30px;color:var(--muted)">' +
      '<p style="font-size:2em">📭</p>' +
      '<p>CDN模式暂不加载实时新闻</p>' +
      '<p style="font-size:0.8em">使用本地服务器 (npm run dashboard) 可获取实时新闻</p></div>';
  }
